...
import (
	...
	"nginx_updata_config/internal/service/git"
	"nginx_updata_config/internal/service/lock"
...
func New(cfg config.Config) (*Runner, error) { return NewWithRuntime(cfg, &NginxRuntime{Config: cfg}) }
func NewWithRuntime(cfg config.Config, rt Runtime) (*Runner, error) {
	store, e := state.Open(cfg.DataDir)
	if e != nil {
		return nil, e
	}
...
func (r *Runner) step(ctx context.Context, st *state.TargetState, rec *state.Record, name string, fn func(context.Context) error) error {
	...
	applog.LogInfo("发布步骤完成", "release_step", map[string]any{"release_id": rec.Request.ReleaseID, "target_id": st.Target.ID, "step": name, "status": status, "error": message})
	return err
}
func (r *Runner) verify(ctx context.Context, t config.Target, base *os.Root, v *state.Version) error {
	...
	if t.Type == release.ReleaseTypeFrontendStatic && v != nil {
		return verifyFrontendHTTP(ctx, t, m)
	}
	return nil
}
...
func (r *Runner) cleanupExport(work string) {
	...
	if e = fsutil.RemoveTree(ctx, root, filepath.Base(work)); e != nil {
		applog.LogError("导出临时目录清理未完成", "cleanup_warning", map[string]any{"error": e.Error()})
	}
}
