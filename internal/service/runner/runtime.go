...
import (
	...
	"nginx_updata_config/internal/service/process"
...
// Runtime is injected in fault tests; the production implementation targets one explicit Nginx instance.
...
func (n *NginxRuntime) args(extra ...string) []string {
...
func (n *NginxRuntime) Test(ctx context.Context) error {
	_, e := process.Run(ctx, "", nil, nil, n.Config.Nginx.Binary, n.args("-t")...)
	return e
}
func (n *NginxRuntime) workers(ctx context.Context) (map[int]bool, error) {
...
func (n *NginxRuntime) Reload(ctx context.Context) error {
	p, e := n.workers(ctx)
	if e != nil {
		return e
	}
...
func (n *NginxRuntime) Verify(ctx context.Context, t config.Target, commit string, initial bool) error {
...
func (r *Runner) verifyRetainedAssets(ctx context.Context, t config.Target, base *os.Root, st *state.TargetState, current *state.Record) error {
	...
	for _, v := range versions {
		...
		for name, digest := range m.Assets {
			...
			if e = probe(ctx, h, v.CommitID, digest); e != nil {
